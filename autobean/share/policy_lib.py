...
class PolicyDefinition:
    ...
    def as_policy(self) -> Optional[Policy]:
        if (
            not self.ownership
            or self.enforced is None
            or self.conversion is None
            or self.prorated_included is None
        ):
            return None
        return Policy(
...
def try_parse_policy_definition(
    ...
    prorate_included = _get_meta(meta, _SpecialMeta.PRORATED_INCLUDED, bool, None)
    if (
        not ownership
        and parent is None
        and balance is None
        and conversion is None
        and prorate_included is None
    ):
        return None
...
class PolicyDatabase:
    ...
    def _get_account_policy_definition(
        self, account: str
    ) -> Optional[PolicyDefinition]:
        policy_def = None
        for parent in reversed(list(beancount_account.parents(account))):
            if wildcard_policy_def := self._wildcard_account_policies.get(parent):
                policy_def = _override_policy_def(
                    self._resolve_parent(wildcard_policy_def),
                    policy_def,
                    is_ephemeral=False,
                )
    ...
    def get_posting_policy(
        ...
        if transaction_policy_def:
            policy_def = _override_policy_def(
                self._resolve_parent(transaction_policy_def),
                policy_def,
                is_ephemeral=True,
            )
    ...
    def get_balance_policy(
        self, balance: Balance
    ) -> Optional[Policy[WeightedOwnership]]:
        ...
        if isinstance(policy_def.ownership, WeightedOwnership) and (
            policy_def.enforced or has_explicit_policy
        ):
            return policy_def.as_policy()
        return None

    def get_proportionate_policy(
        self, custom: Custom, account: str
    ) -> Policy[WeightedOwnership]:
        policy_def = _ROOT_POLICY
        if default_policy_def := self._named_policies.get("default"):
            policy_def = _override_policy_def(
                self._resolve_parent(default_policy_def), policy_def, is_ephemeral=False
    ...
    *,
    ...
    if is_ephemeral:
        ...
        if parent.enforced and policy_def.enforced == False:
            raise error_lib.PluginException("Cannot unset share_enforced")
    ownership = policy_def.ownership or parent.ownership
    balance = (
        policy_def.enforced if policy_def.enforced is not None else parent.enforced
    )
    conversion = (
        policy_def.conversion
        if policy_def.conversion is not None
        else parent.conversion
    )
    prorate_included = (
        policy_def.prorated_included
        if policy_def.prorated_included is not None
        else parent.prorated_included
    )
    return PolicyDefinition(
